import { BOARD_ROWS, BOARD_COLS, ROW_LABELS, SHIPS } from '../constants/ships';

export const idx = (r, c) => r * BOARD_COLS + c;
export const rc = (i) => ({ r: Math.floor(i / BOARD_COLS), c: i % BOARD_COLS });
...
export function getShipCells(pos, size, horizontal) {
  ...
  for (let k = 0; k < size; k++) {
    ...
    if (nr < 0 || nr >= BOARD_ROWS || nc < 0 || nc >= BOARD_COLS) return null;
...
export function generateRandomFleet(shipsDef = SHIPS) {
  const board = Array(BOARD_ROWS * BOARD_COLS).fill(null);
  const ships = [];
  for (let si = 0; si < shipsDef.length; si++) {
    let placed = false;
    while (!placed) {
      const horizontal = Math.random() < 0.5;
      const pos = Math.floor(Math.random() * BOARD_ROWS * BOARD_COLS);
